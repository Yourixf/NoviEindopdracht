...
def formateer_film_lijst(response=None, gebruiker_film_titel=None):
    """
    Deze functie formateert de film lijst response.

    Deze functie kijkt eerst of er resultaten zijn, en indien er wel resultaten zijn word deze op een duidelijke
    manier weergegeven aan de gebruiker. Daarna zal de functie een boolean waarden en response retourneren.
    """
    ...
    if response is None:
        ...
        for movie in response_dict.get("results", []):
            ...
            release_date = api_data_handling_general.formateer_maximale_grootte(
                movie.get("release_date", "").strip() or "ONBEKEND")
            overview = api_data_handling_general.formateer_maximale_grootte(
                movie.get("overview", "").strip() or "ONBEKEND")
            movie_id = api_data_handling_general.formateer_maximale_grootte(
...
def formateer_film_details(response=None, gebruiker_film_ID=None):
    ...
    Deze functie krijgt een API response van een film detail call, dan al deze functie over de data itereren,
    vervolgens formateren en presenteren aan de gebruiker. Tot slot zal deze een boolean waarde retourneren aan de
    aanroepende functie.
    """
...
def formateer_genre_film_lijst(response=None, gebruiker_genre_id=None):
    ...
    if response is None:
        print("\nEr gins wat mis... Ik heb geen data meegekregen.")
        if config.terminal_logging:
...
def formateer_release_datum_film_lijst(response=None, gebruiker_release_datum=None):
    ...
    Deze functie wordt aangeroepen vanuit filter_genre(). De functie zal over de data itereren, vervolgens formateren
    en presenteren aan de gebruiker als het gekozen jaartal in de repsonse zit. Tot slot zal deze een boolean waarde
    en de response retourneren aan de aanroepende functie.
    """
    ...
    if response is None:
        print("\nEr gins wat mis... Ik heb geen data meegekregen.")
        if config.terminal_logging:
            print("Logging - Geen film lijst via gekozen jaartal weergegeven\n")
        return False
    elif response_dict.get("total_results", 0) == 0:
        print(f"Geen film(s) met jouw gekozen jaartal.\n")
        # Word geprint als logging variabele in main.py op True staat.
        if config.terminal_logging:
            print("Logging - Geen film lijst via gekozen jaartal weergegeven\n")
        return False
